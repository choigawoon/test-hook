...
const cwd = process.env.CLAUDE_PROJECT_DIR || input.cwd || input.workspace_path || input.workspacePath || process.cwd();
...
if (!projectRoot) {
  if (reportNoop) {
    emitClaudeContext(`Claude project hook reached with Node.js ${nodeVersion}; skipped mLua LSP start because no RootDesk was found from ${cwd}.`);
  }
  process.exit(0);
}
...
if (result.error || result.status !== 0) {
  ...
  emitClaudeContext(`Claude project hook reached with Node.js ${nodeVersion}, but mLua LSP daemon start failed. Ask the AI agent to install/fix mlua-lsp, or install Node.js from https://nodejs.org/ and then run npm install -g @maplestoryworlds/mlua-lsp. Detail: ${detail}`);
  process.exit(0);
}

if (reportStart) {
  emitClaudeContext(`Claude project hook reached with Node.js ${nodeVersion}; mLua LSP daemon is ready for ${projectRoot}.`);
}
