...
function emitVsCodeContext(message, systemMessage) {
  const output = {
    continue: true,
    hookSpecificOutput: {
...
const cwd = input.cwd || input.workspace_path || input.workspacePath || process.cwd();
const projectRoot = process.env.MLUA_LSP_PROJECT_ROOT || findProjectRoot(cwd);
...
if (!projectRoot) {
  if (reportNoop) {
    emitVsCodeContext(`VS Code Copilot hook reached with Node.js ${nodeVersion}; skipped mLua LSP start because no RootDesk was found from ${cwd}.`);
  } else {
...
if (result.error || result.status !== 0) {
  const detail = String(result.error?.message || result.stderr || `exit ${result.status}`).trim();
  emitVsCodeContext(
    `VS Code Copilot hook reached with Node.js ${nodeVersion}, but mLua LSP daemon start failed. Ask the AI agent to install/fix mlua-lsp, or install Node.js from https://nodejs.org/ and then run npm install -g @maplestoryworlds/mlua-lsp. Detail: ${detail}`,
    'mLua LSP daemon start failed. Check the GitHub Copilot Chat Hooks output channel.'
  );
  process.exit(0);
}

if (reportStart) {
  emitVsCodeContext(`VS Code Copilot hook reached with Node.js ${nodeVersion}; mLua LSP daemon is ready for ${projectRoot}.`);
} else {
  process.stdout.write(JSON.stringify({ continue: true }));
}
