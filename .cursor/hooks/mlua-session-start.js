...
if (!projectRoot) {
  if (reportNoop) {
    emitCursorContext(`Cursor project hook reached with Node.js ${nodeVersion}; skipped mLua LSP start because no RootDesk was found from ${cwd}.`);
  } else {
...
if (result.error || result.status !== 0) {
  ...
  emitCursorContext(`Cursor project hook reached with Node.js ${nodeVersion}, but mLua LSP daemon start failed. Ask the AI agent to install/fix mlua-lsp, or install Node.js from https://nodejs.org/ and then run npm install -g @maplestoryworlds/mlua-lsp. Detail: ${String(message).trim()}`);
  process.exit(0);
}

if (reportStart) {
  emitCursorContext(`Cursor project hook reached with Node.js ${nodeVersion}; mLua LSP daemon is ready for ${projectRoot}.`);
} else {
  process.stdout.write(JSON.stringify({ continue: true, permission: 'allow' }));
}
